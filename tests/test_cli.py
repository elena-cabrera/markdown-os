...
def test_web_command_programmatic_call_uses_web_runtime(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """
    Verify direct Python calls launch web mode without a filesystem handler.

    Args:
    - monkeypatch (pytest.MonkeyPatch): Fixture used to replace runtime dependencies.

    Returns:
    - None: Assertions validate web runtime server initialization.
    """
    ...
    def _fake_find_available_port(host: str = "127.0.0.1", start_port: int = 8000) -> int:
        """
        Capture host and port inputs used for web server port discovery.
    ...
    def _fake_create_app(*, mode: str, handler: object, desktop: bool = False) -> object:
        """
        Capture app factory inputs and return a fake application object.

        Args:
        - mode (str): Mode passed to app factory.
        - handler (object): Filesystem handler, expected to be None in web mode.
        - desktop (bool): Desktop flag passed to app factory.
        ...
        observed["create_app_handler"] = handler
        observed["create_app_desktop"] = desktop
        return {"app": "web"}
    ...
    monkeypatch.setattr(cli_module, "build_editor_app", _fake_create_app)
    monkeypatch.setattr(cli_module.uvicorn, "run", _fake_uvicorn_run)

    cli_module.open_web_editor()

    assert observed["host"] == "127.0.0.1"
    assert observed["start_port"] == 8000
    assert observed["url"] == "http://127.0.0.1:8124"
    assert observed["create_app_mode"] == "web"
    assert observed["create_app_handler"] is None
    assert observed["create_app_desktop"] is False
    assert observed["uvicorn_host"] == "127.0.0.1"
    assert observed["uvicorn_port"] == 8124
